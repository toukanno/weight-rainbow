  ...
  app.innerHTML = `
    ...
    <div class="rainbow-overlay" id="rainbowOverlay">
      ...
      <div class="rainbow-card">
        <div class="rainbow-emoji">\u{1F308}\u2728</div>
...
function saveRecordWithWeight(weight, source) {
  ...
  setStatus(`${t("entry.saved")} \xB7 ${t("bmi.title")}: ${record.bmi ? record.bmi.toFixed(1) : t("bmi.unknown")}`);
}
...
function exportData() {
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = `weight-rainbow-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
  anchor.click();
  URL.revokeObjectURL(url);
